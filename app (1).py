...
3) NUEVA PESTAÑA "🌐 Sensibilidad de η a Eg y T": dos cortes 2D interactivos
   (η vs T de operación a Eg fijo, η vs Eg a T fija) más la superficie 3D
   completa η(Eg,T). No agrega ninguna ecuación nueva: reutiliza exactamente
   ni_T_cm3, Voc_limite_auger_V y FF_con_parasiticos, con los valores de
   Rs, Rp, factor de colección y factor óptico que el usuario tenga puestos
   en ese momento en la Pestaña 2 (st.session_state), y marca en rojo el
   punto (Eg, T) que el usuario esté explorando ahora mismo en las Pestañas
   1 y 2/3 -- se recalcula solo cada vez que se mueven esos sliders. Rango
   de Eg limitado a 0.5-2.2 eV: la fórmula empírica de FF con parásitos
   (Green, 1981) deja de ser válida por sobre ese valor (ver caption de la
   propia pestaña).
"""
...
# ============================================================================
# PESTAÑA 6 — Sensibilidad de eta(Eg, T)
# ============================================================================
with tab6:
    st.header("Sensibilidad de la eficiencia a Eg y T")
    st.markdown(
        "$J_{sc}$, $V_{OC}$ y $FF$ no son ejes independientes: los tres se derivan de "
        "los dos parámetros que realmente exploras en esta app — **Eg** (Pestaña 1) y la "
        "**temperatura de operación** (Pestaña 2/3). Los tres gráficos de abajo usan las "
        "mismas funciones de `semiconductor.py`/`espectro.py` que la cascada de la "
        "Pestaña 2 (con los mismos Rs, Rp, factor de colección y factor óptico que tengas "
        "puestos ahí ahora mismo), y se recalculan solos cada vez que mueves esos sliders "
        "en cualquier otra pestaña — el punto rojo es tu exploración actual."
    )

    def _parametros_actuales_pestana6():
        """Mismos parametros de mecanismos 5-7 que usa calcular_cascada() en la Pestaña 2,
        leidos en vivo desde st.session_state (no valores fijos)."""
        return (
            st.session_state.Rs_ohmcm2,
            st.session_state.Rp_ohmcm2,
            st.session_state.factor_coleccion,
            st.session_state.factor_optico,
        )

    def _eta_punto_pestana6(Eg, T_C):
        """eta(%) en un (Eg, T) puntual -- misma logica que calcular_cascada()."""
        Rs_p6, Rp_p6, fcol_p6, fopt_p6 = _parametros_actuales_pestana6()
        T_K = T_C + 273.15
        Jsc_p6 = espectro.Jsc_max_mA_cm2(Eg)
        ni_p6 = ni_T_cm3(T_K, Eg_eV=Eg)
        Voc_p6 = Voc_limite_auger_V(Jsc_p6, T_K, W_cm, ni_p6)
        FF_p6, _, _ = FF_con_parasiticos(Voc_p6, Jsc_p6, T_K, Rs_p6, Rp_p6)
        Jsc_efectivo_p6 = Jsc_p6 * fcol_p6 * fopt_p6
        return Voc_p6 * Jsc_efectivo_p6 * FF_p6 / (P_AM1_5_W_M2 / 10.0) * 100.0

    @st.cache_data(show_spinner="Calculando superficie η(Eg, T)...")
    def _grid_eta_superficie(Rs_p6, Rp_p6, fcol_p6, fopt_p6):
        Eg_arr = np.linspace(0.5, 2.2, 70)
        T_C_arr = np.linspace(15.0, 75.0, 55)
        Eg_grid, T_C_grid = np.meshgrid(Eg_arr, T_C_arr)
        T_K_grid = T_C_grid + 273.15

        Jsc_por_Eg = np.array([espectro.Jsc_max_mA_cm2(eg) for eg in Eg_arr])
        Jsc_grid = np.tile(Jsc_por_Eg, (len(T_C_arr), 1))

        ni_grid = ni_T_cm3(T_K_grid, Eg_eV=Eg_grid)
        Voc_grid = Voc_limite_auger_V(Jsc_grid, T_K_grid, W_cm, ni_grid)
        FF_real_grid, _, _ = FF_con_parasiticos(Voc_grid, Jsc_grid, T_K_grid, Rs_p6, Rp_p6)

        Jsc_efectivo = Jsc_grid * fcol_p6 * fopt_p6
        eta_grid = (Voc_grid * Jsc_efectivo * FF_real_grid) / (P_AM1_5_W_M2 / 10.0) * 100.0
        return Eg_arr, T_C_arr, eta_grid

    @st.cache_data(show_spinner=False)
    def _curva_eta_vs_T_p6(Eg_fijo, Rs_p6, Rp_p6, fcol_p6, fopt_p6):
        T_C_line = np.linspace(15.0, 75.0, 150)
        return T_C_line, np.array([_eta_punto_pestana6(Eg_fijo, t) for t in T_C_line])

    @st.cache_data(show_spinner=False)
    def _curva_eta_vs_Eg_p6(T_fijo, Rs_p6, Rp_p6, fcol_p6, fopt_p6):
        Eg_line = np.linspace(0.5, 2.2, 150)
        return Eg_line, np.array([_eta_punto_pestana6(eg, T_fijo) for eg in Eg_line])

    # Punto actualmente explorado por el usuario (estado compartido entre pestañas)
    Eg_actual_p6 = st.session_state.Eg_eV
    T_actual_p6 = st.session_state.T_operacion_C
    params_p6 = _parametros_actuales_pestana6()
    eta_actual_p6 = _eta_punto_pestana6(Eg_actual_p6, T_actual_p6)

    # --- Fila superior: dos cortes 2D interactivos ---
    col_p6a, col_p6b = st.columns(2)

    with col_p6a:
        T_line_p6, eta_vs_T_p6 = _curva_eta_vs_T_p6(Eg_actual_p6, *params_p6)
        fig_T_p6 = go.Figure()
        fig_T_p6.add_trace(go.Scatter(
            x=T_line_p6, y=eta_vs_T_p6, mode="lines",
            line=dict(color="#1F6F8B", width=3),
            hovertemplate="T=%{x:.0f}°C<br>η=%{y:.2f}%<extra></extra>",
        ))
        fig_T_p6.add_trace(go.Scatter(
            x=[T_actual_p6], y=[eta_actual_p6], mode="markers",
            marker=dict(size=12, color="#A32626", line=dict(color="white", width=2)),
            hovertemplate="Tu punto actual<br>T=%{x:.0f}°C<br>η=%{y:.2f}%<extra></extra>",
        ))
        fig_T_p6.update_layout(
            title=dict(text=f"η vs T de operación<br><sub>Eg fijo = {Eg_actual_p6:.2f} eV (Pestaña 1)</sub>",
                       x=0.02, xanchor="left"),
            xaxis_title="T de operación [°C]", yaxis_title="η [%]",
            height=380, margin=dict(l=55, r=15, t=70, b=45),
            showlegend=False, plot_bgcolor="white",
        )
        fig_T_p6.update_xaxes(gridcolor="#EAEDF0")
        fig_T_p6.update_yaxes(gridcolor="#EAEDF0")
        st.plotly_chart(fig_T_p6, width="stretch")
        pend_p6 = (eta_vs_T_p6[-1] - eta_vs_T_p6[0]) / (T_line_p6[-1] - T_line_p6[0])
        st.caption(f"A Eg={Eg_actual_p6:.2f} eV fijo, η cae de {eta_vs_T_p6[0]:.1f}% (15°C) a "
                   f"{eta_vs_T_p6[-1]:.1f}% (75°C) — pendiente promedio ≈ {pend_p6:.3f} %/°C.")

    with col_p6b:
        Eg_line_p6, eta_vs_Eg_p6 = _curva_eta_vs_Eg_p6(T_actual_p6, *params_p6)
        fig_Eg_p6 = go.Figure()
        fig_Eg_p6.add_trace(go.Scatter(
            x=Eg_line_p6, y=eta_vs_Eg_p6, mode="lines",
            line=dict(color="#B5790A", width=3),
            hovertemplate="Eg=%{x:.2f} eV<br>η=%{y:.2f}%<extra></extra>",
        ))
        fig_Eg_p6.add_trace(go.Scatter(
            x=[Eg_actual_p6], y=[eta_actual_p6], mode="markers",
            marker=dict(size=12, color="#A32626", line=dict(color="white", width=2)),
            hovertemplate="Tu punto actual<br>Eg=%{x:.2f} eV<br>η=%{y:.2f}%<extra></extra>",
        ))
        fig_Eg_p6.update_layout(
            title=dict(text=f"η vs banda prohibida Eg<br><sub>T fija = {T_actual_p6:.0f}°C (Pestaña 2/3)</sub>",
                       x=0.02, xanchor="left"),
            xaxis_title="Eg explorado [eV]", yaxis_title="η [%]",
            height=380, margin=dict(l=55, r=15, t=70, b=45),
            showlegend=False, plot_bgcolor="white",
        )
        fig_Eg_p6.update_xaxes(gridcolor="#EAEDF0")
        fig_Eg_p6.update_yaxes(gridcolor="#EAEDF0")
        st.plotly_chart(fig_Eg_p6, width="stretch")
        i_max_p6 = int(np.argmax(eta_vs_Eg_p6))
        st.caption(f"A T={T_actual_p6:.0f}°C fija, el óptimo de este barrido está en "
                   f"Eg={Eg_line_p6[i_max_p6]:.2f} eV (η={eta_vs_Eg_p6[i_max_p6]:.1f}%) — el "
                   f"silicio (1.12 eV) está "
                   f"{'a la izquierda' if Eg_actual_p6 < Eg_line_p6[i_max_p6] else 'a la derecha'} "
                   "de ese óptimo.")

    st.divider()

    # --- Fila inferior: superficie 3D completa ---
    st.subheader("Superficie completa η(Eg, T)")
    Eg_arr_p6, T_C_arr_p6, eta_grid_p6 = _grid_eta_superficie(*params_p6)

    fig3d_p6 = go.Figure(data=[go.Surface(
        x=Eg_arr_p6, y=T_C_arr_p6, z=eta_grid_p6, colorscale="Viridis",
        colorbar=dict(title="η [%]"),
        contours={"z": {"show": True, "usecolormap": True, "project_z": False}},
    )])
    fig3d_p6.add_trace(go.Scatter3d(
        x=[Eg_actual_p6], y=[T_actual_p6], z=[eta_actual_p6],
        mode="markers+text", marker=dict(size=6, color="red"),
        text=[f"Tu exploración actual<br>Eg={Eg_actual_p6:.2f} eV, T={T_actual_p6:.0f}°C<br>η={eta_actual_p6:.1f}%"],
        textposition="top center", name="Estado actual",
    ))
    fig3d_p6.update_layout(
        scene=dict(
            xaxis_title="Eg explorado [eV]",
            yaxis_title="T de operación [°C]",
            zaxis_title="Eficiencia η [%]",
        ),
        height=650, margin=dict(l=0, r=0, t=10, b=0),
    )
    st.plotly_chart(fig3d_p6, width="stretch")

    st.caption(
        "El eje Eg se limita a 2.2 eV en los tres gráficos: sobre ese valor, Jsc cae bajo "
        "~10 mA/cm² y la fórmula empírica de FF con parásitos (Green, 1981) deja de ser "
        "válida con los Rs/Rp actuales (puede dar FF negativo, un artefacto numérico de la "
        "aproximación, no un resultado físico). Los tres gráficos usan los mismos Rs, Rp, "
        "factor de colección y factor óptico que tengas puestos ahora mismo en la Pestaña 2."
    )
